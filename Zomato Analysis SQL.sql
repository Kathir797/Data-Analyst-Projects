...
case when monthname(datekey_opening) in ('January' ,'February' ,'March' )then 'F-Q4'
when monthname(datekey_opening) in ('April' ,'May' ,'June' )then 'F-Q1'
when monthname(datekey_opening) in ('July' ,'August' ,'September' )then 'F-Q2'
else  'F-Q3' end as Financial_quarters
...
select  Countryname,RestaurantName,max(Rating)highest_rating from main inner join country on main.CountryCode=country.CountryID
group by country.Countryname,RestaurantName;

----------------------------

# top 5 restaurants who has more number of votes

select  Countryname,RestaurantName,Votes,Average_Cost_for_two from main inner join country on main.CountryCode=country.CountryID
...
select  Countryname,RestaurantName,max(Rating)highest_rating,max(Votes) from main inner join country on main.CountryCode=country.CountryID
group by country.Countryname,RestaurantName,Votes order by max(Votes) desc limit 5;
